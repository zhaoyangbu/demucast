...
from models.demucs_tool import DConv, rescale_module, capture_init, spectro, demucs2ast
...
class HDemucs(nn.Module):
    ...
    def forward(self, mix):
        ...
        if self.hybrid:
            ...
            for i in range(S):
                xt_temp = torch.squeeze(xt[:,i,:,:], 1)
                xf_temp = self._spec(xt_temp)
                xf_temp = torch.unsqueeze(xf_temp, 1)
                if i == 0:
                    xf_2 = xf_temp
                else:
                    xf_2 = torch.cat((xf_2,xf_temp), 1)


            if self.audio_channels == 1:
                xf_2 = xf_2.unsqueeze(dim=2)
            else:
                pass
            x = xf_1 + xf_2
            x = demucs2ast(x, mono=self.mono)
        return x


if __name__ == '__main__':
    SOURCES = ["drums", "bass", "other", "vocals"]
    demucs = HDemucs(sources=SOURCES, audio_channels=2)
    test_input = torch.rand([6, 2, 480000])
    test_output = demucs(test_input)
    print(test_output.size()) #torch.Size([4, 4, 2, 2048, 469]) --> torch.Size([4, 1876, 128])